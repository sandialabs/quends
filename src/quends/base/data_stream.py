...
class DataStream:
    ...
    def _process_column(self, column_data, estimated_window, method):
        ...
        if method == "sliding":
            ...
            window_means = [np.mean(column_data[i:i + estimated_window])
                            for i in range(0, len(column_data) - estimated_window + 1, step_size)]
            return pd.Series(window_means,
                             index=np.arange(estimated_window // 2,
    ...
    def trim(self, column_name, window_size=10, start_time=0.0, method="std", threshold=None, robust=True):
        ...
        """
        if method == "std":
            steady_state_start_time = self.find_steady_state_std(self.df, column_name, window_size, start_time, robust)
        elif method == "threshold":
            if threshold is None:
                raise ValueError("Threshold must be specified for the 'threshold' method.")
            steady_state_start_time = self.find_steady_state_threshold(self.df, column_name, window_size, threshold, start_time)
        elif method == "rolling_variance":
            threshold = threshold if threshold is not None else 0.1
            steady_state_start_time = self.find_steady_state_rolling_variance(self.df, column_name, window_size, threshold)
        else:
    ...
    @staticmethod
    def find_steady_state_std(data, column_name, window_size=10, start_time=0.0, robust=True):
        """
    ...
    @staticmethod
    def find_steady_state_rolling_variance(data, column_name, window_size=50, threshold=0.1):
        """
    ...
    @staticmethod
    def find_steady_state_threshold(data, column_name, window_size, threshold, start_time=0.0):
        """
        ...
        rolling_std = filtered[column_name].rolling(window=window_size).std().rolling(window=window_size).std() / np.sqrt(window_size)
        # Align indices between rolling_std and time
        common_idx = filtered.index.intersection(rolling_std.index)
        steady_state = filtered.loc[common_idx, 'time'][rolling_std.loc[common_idx] < threshold]
    ...
    def effective_sample_size(self, column_names=None, alpha=0.05):
        ...
        for col in column_names:
            if col not in self.df.columns:
                results[col] = {"message": f"Column '{col}' not found in the DataStream."}
                continue

            filtered = self.df[col].dropna()
            if filtered.empty:
                results[col] = {"effective_sample_size": None, "message": "No data available for computation."}
                continue
    ...
    def confidence_interval(self, column_name=None, ddof=1, method="non-overlapping", window_size=None):
        ...
        uncertainty_results = self.mean_uncertainty(column_name, ddof=ddof, method=method, window_size=window_size)
    ...
    def compute_statistics(self, column_name=None, ddof=1, method="non-overlapping", window_size=None):
        """
        ...
        """
        mean_results = self.mean(column_name, method=method, window_size=window_size)
        mu_results = self.mean_uncertainty(column_name, ddof=ddof, method=method, window_size=window_size)
        ci_results = self.confidence_interval(column_name, ddof=ddof, method=method, window_size=window_size)

        statistics = {}
        for col in mean_results.keys():
            if col not in mean_results or col not in mu_results or col not in ci_results:
                statistics[col] = {"error": f"Missing data for column '{col}'"}
                continue

            ci = ci_results[col].get("confidence interval")
            if ci is None:
                statistics[col] = {"error": f"Confidence interval not computed for column '{col}'"}
                continue
            ...
            statistics[col] = {
                ...
                "pm_std": (mean_results[col]["mean"] - mu_results[col]["mean uncertainty"],
    ...
    def optimal_window_size(self, column_name=None, method="non-overlapping"):
        ...
        for col in self._get_columns(column_name):
            ...
            for window in range(1, max_window_size, 5):
                stats_res = self.compute_statistics(column_name=col, method=method, window_size=window)
            ...
            optimal_window = min(std_results, key=std_results.get) if std_results else None
            results[col] = {
    ...
    def additional_data(self, column_name=None, ddof=1, method="sliding", window_size=None, reduction_factor=0.1):
        """
        ...
        stats = self.cumulative_statistics(column_name, method=method, window_size=window_size)
        results = {}
