    ...
    def process_data_steam(self, data_stream_orig, col, start_time=0.0):
        ...
        * look at number of effective samples we have. Could be low. Allow user to
        ...
        if stationary:
            ...
            col_smoothed = (
                ds_wrk.df[col].rolling(window=rolling_window).mean()
            ...
            std_dev_smoothed = std_dev_till_end_series.rolling(
                window=self._final_smoothing_window
            ).mean()
            ...
            df_std_dev = pd.DataFrame(
                {"time": ds_wrk.df["time"], col + "_std_till_end": std_dev_sm_flld}
            )
            ...
            deviation = pd.DataFrame(
                {"time": ds_wrk.df["time"], col + "_deviation": deviation_sm_flld}
            )
            ...
            tol_fac = self._std_dev_frac * (
                df_std_dev[col + "_std_till_end"] + self._fudge_fac * abs(mean_vals[0])
            )
            ...
            if crit_met_index is not None:  # We have a SSS segment
                ...
                true_sss_start_index = max(
                    ...
                    int(
                        crit_met_index
                        - self._smoothing_window_correction * rolling_window
                    ),
                )
