...
def create_netcdf_file():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with nc.Dataset(test_file, "w", format="NETCDF4") as dataset:
            ...
            heatflux_var[:] = np.random.uniform(
                low=15.0, high=30.0, size=10
            )  # Random heat flux values
            wg_var[:] = np.random.uniform(
                low=0.0, high=10.0, size=10
            )  # Random wg values
            wphi_var[:] = np.random.uniform(
                low=0.0, high=360.0, size=10
            )  # Random wphi values
...
def test_without_variables(create_netcdf_file):
    (
        ...
        wg_values,
        wphi_values,
    ) = create_netcdf_file  # Get the filename and original values
