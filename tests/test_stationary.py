...
def test_is_stationary(stationary_data: pd.DataFrame):
    ...
    # Constant series: ADF raises an exception internally; is_stationary returns False.
...
def test_make_stationary_with_stationary_data(stationary_data: pd.DataFrame):
    ...
    # Constant series: ADF raises internally; is_stationary returns False.
    # With operate_safe=False and the loop conditions, no data is dropped.
    assert stationary is False or stationary == False  # noqa: E712
...
def test_make_stationary_already_stationary(stationary_noise_df: pd.DataFrame):
    ...
    # is_stationary returns a plain bool; no .any() needed.
    assert stationary is True or stationary == True  # noqa: E712
