...
def test_from_csv_with_variables(create_csv_file):
    ...
    np.testing.assert_array_equal(df['time'].values, expected_data['time'], "Time values do not match.")
    np.testing.assert_array_equal(df['HeatFlux_st'].values, expected_data['HeatFlux_st'], "HeatFlux_st values do not match.")
